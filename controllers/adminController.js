...
module.exports = {
    home: async(req, res, next) => {
        try {
             const revenue = await Order.aggregate([{ $match: { $or: [{ $and: [{ paymentMethod: 'COD', orderStatus: 'delivered' }] }, { $and: [{ paymentMethod: 'razorpay', orderStatus: 'placed' }] }] } },
            {
                $group: {
                    _id: {
                        id: null
            ...
            const TodayRevenue = await Order.aggregate([{ $match: { $or: [{ $and: [{ paymentMethod: 'COD', orderStatus: 'delivered' }] }, { $and: [{ paymentMethod: 'razorpay', orderStatus: 'placed' }] }] } },
            {
            ...
            res.render('admin/index',{ revenue,Allsales,TodayRevenue,TodaySale })
        } catch (error) {
            console.log(error)
            error.admin = true
            next(error)
    ...
    salesReport: async (req, res, next) => {
        try {
            console.log(req.body);
            console.log(req.body.from + " " + "GMT");
            console.log(req.body.from);
            let from = req.body.from
            from = from + " " + "GMT"
            console.log(new Date(from), 'kuttaaaa');
            const sales = await Order.aggregate([{
                $match: {
                    orderStatus: { $eq: 'delivered' }, $and: [
                        { date: { $gt: new Date(req.body.from + "GMT") } },
                        { date: { $lt: new Date(req.body.to + "GMT") } }
                    ]
                }
            }, {
                $group: {
            ...
            res.render('admin/salesDateReport', { sales })
        } catch (error) {
            console.log(error)
            error.admin = true
            next(error)
    ...
    yearlySalesReport: async (req, res,next) => {
       try {
        const sales = await Order.aggregate([{
            $match: { orderStatus: { $eq: 'delivered' } }
        }, {
